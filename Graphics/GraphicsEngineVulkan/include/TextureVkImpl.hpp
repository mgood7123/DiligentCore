...
public:
    ...
    // For non-compressed color format buffer offset must be a multiple of the format's texel block size.
    // For compressed format buffer offset must be a multiple of the compressed texel block size in bytes.
    // For depth-stencil format buffer offset must be a multiple of 4.
    // If command buffer does not support graphics or compute commands then buffer offset must be a multiple of 4.
