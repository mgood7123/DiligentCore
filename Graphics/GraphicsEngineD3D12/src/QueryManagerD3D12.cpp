                                     ...
                                     HardwareQueueIndex     HwQueueInd)
{
    ...
    for (Uint32 QueryType = QUERY_TYPE_UNDEFINED + 1; QueryType < QUERY_TYPE_NUM_TYPES; ++QueryType)
    {
        ...
        if (QUERY_TYPE_TIMESTAMP || QueryType == QUERY_TYPE_DURATION)
        {
            if (HwQueueInd == CopyQueueId && !DevInfo.Features.TransferQueueTimestampQueries)
                continue; // Not supported in transfer queue
        }
        // Other queries supported only in graphics queue
        else if (HwQueueInd != GraphicsQueueId)
            continue;
