...
struct DeviceFeatures
{
    ...
    /// Indicates if device supports timestamp and duration queries in transfer queue.
...
DILIGENT_TYPED_ENUM(RESOURCE_STATE, Uint32)
{
    ...
    /// The resource state is used for read operations, but access to resource may be slower than when used specialized resource state.
    ...
    /// \remarks In D3D12 backend resource must be in COMMON state for transition between graphics/compute queue and copy queue.
    /// \remarks Supported contexts: graphics, compute, transfer.
